...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::call_index(2)]
		#[pallet::weight(0)]
		pub fn transfer_claim(origin: OriginFor<T>, to: T::AccountId, claim: BoundedVec<u8, T::MaxClaimLength>) -> DispatchResultWithPostInfo {
			let sender = ensure_signed(origin)?;

			let old_value = Proofs::<T>::get(&claim).ok_or(Error::<T>::ClaimNotExist)?;
			ensure!(old_value.0 == sender, Error::<T>::NotClaimOwner);

			//Proofs::<T>::mutate(&claim, |old_value| (to.clone(), frame_system::Pallet::<T>::block_number()));
			Proofs::<T>::remove(&claim);
